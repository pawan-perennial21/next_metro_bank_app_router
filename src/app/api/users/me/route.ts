...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { companyName, email, oldPassword, newPassword, confirmPassword } =
      reqBody;
    ...
    if (oldPassword && newPassword && confirmPassword) {
      ...
      existingUser.password = await bcryptjs.hash(newPassword, salt);
      ...
      const response = SuccessResponse("User updated successfully", 200);
